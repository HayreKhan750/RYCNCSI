...
import { toggleLike } from '../../store/slices/feedbackSlice';
...
const ReviewList = ({ reviews }) => {
  ...
  const handleLike = async (reviewId) => {
    ...
    dispatch(toggleLike({ feedbackId: reviewId, userId: user.uid, isLike: true }));
  };
  ...
  const renderReviewItem = (review, index) => {
    const isMyReview = user && review.studentId === user.uid;
    const hasReplies = review.replies && review.replies.length > 0;
    const canReply = !isMyReview || hasReplies;
      ...
      <div key={review.id} className="glass-card review-card" style={{ 
        ...
        <div style={{ display: 'flex', gap: '15px', marginTop: 'auto', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '15px' }}>
          ...
          style={{ 
            ...
            alignItems: 'center', 
            gap: '5px', 
            color: review.likedBy?.includes(user?.uid) ? 'var(--primary)' : 'inherit', 
            opacity: isMyReview ? 0.5 : (review.likedBy?.includes(user?.uid) ? 1 : 0.8),
            fontWeight: review.likedBy?.includes(user?.uid) ? 'bold' : 'normal'
          }}
          title={isMyReview ? "You cannot like your own review" : (review.likedBy?.includes(user?.uid) ? "Unlike" : "Like")}
        >
          ...
          <button 
            ...
            style={{ 
                ...
                gap: '5px', 
                color: 'inherit', 
                opacity: canReply ? 0.8 : 0.3 
            }}
            title={!canReply ? "You cannot be the first to reply to your own review" : "Reply"}
          >
            <span>💬</span> Reply {review.replies?.length > 0 && `(${review.replies.length})`}
          </button>
